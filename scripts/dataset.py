...
class ChessRecognitionDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.corners = pd.DataFrame(
            annotations_file["annotations"]["corners"],
            index=None)
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        img_anns = F.one_hot(img_anns, num_classes=13).float()  # shape: (64, 13)

        return (img, img_anns)
    
    # ------------------------------------------------------------- #
    #  Utilities (do NOT affect training)
    # ------------------------------------------------------------- #
    def get_raw_image(self, index: int, to_rgb: bool = True):
        """
        Return the full‑resolution image as a NumPy array in [0,1].

        Args:
            index (int): Dataset index.
            to_rgb (bool): If True convert BGR→RGB (TorchVision read_image loads RGB already,
                        so usually leave as True for matplotlib).

        Returns:
            img_np (H, W, 3) float32 in range [0,1]
        """
        img_id = self.split_img_ids[index]
        img_path = Path(
            self.dataroot,
        ...
        img = read_image(str(img_path)).permute(1, 2, 0).float() / 255.0
        return img.numpy()


print(f"Chess Recognition Dataset Created Here")